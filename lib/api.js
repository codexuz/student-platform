...
export const authAPI = {
  login: async (username, password, role = "student") => {
    const loginUrl =
      role === "teacher" ? "/auth/teacher/login" : "/auth/student/login";
...
export const ieltsAPI = {
  ...
  getReadingParts: ({ page = 1, limit = 10, part = "" } = {}) => {
    const params = new URLSearchParams({ page, limit });
    if (part) params.append("part", part);
    return request({
  ...
  getListeningParts: ({ page = 1, limit = 10, part = "" } = {}) => {
    const params = new URLSearchParams({ page, limit });
    if (part) params.append("part", part);
    return request({
  ...
  getWritingTasks: ({ page = 1, limit = 10, task = "" } = {}) => {
    const params = new URLSearchParams({ page, limit });
