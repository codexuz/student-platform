...
export const notificationsAPI = {
  getAllNotificationsWithUnseenCount: () =>
    request({
      url: `/notifications/user/${userStore.getUserProfile.user_id || userStore.getUserProfile.id
        }`,
    }),

  markAsSeen: (notificationId) =>
    request({
      url: `/notifications/seen/${notificationId}/user/${userStore.getUserProfile.user_id || userStore.getUserProfile.id
        }`,
      method: "PATCH",
    }),
...
export const ieltsAPI = {
  ...
  getReadingParts: ({ page = 1, limit = 10, part = "", mode = "", search = "" } = {}) => {
    const params = new URLSearchParams({ page, limit });
    if (part) params.append("part", part);
    if (mode) params.append("mode", mode);
    if (search) params.append("search", search);
  ...
  getListeningParts: ({ page = 1, limit = 10, part = "", mode = "", search = "" } = {}) => {
    const params = new URLSearchParams({ page, limit });
    if (part) params.append("part", part);
    if (mode) params.append("mode", mode);
    if (search) params.append("search", search);
  ...
  getWritingTasks: ({ page = 1, limit = 10, task = "", mode = "", search = "" } = {}) => {
    const params = new URLSearchParams({ page, limit });
