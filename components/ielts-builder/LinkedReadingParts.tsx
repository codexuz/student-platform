...
import { ieltsReadingAPI, ieltsReadingPartsAPI } from "@/lib/ielts-api";
import { toaster } from "@/components/ui/toaster";
import LinkExistingItemModal from "./LinkExistingItemModal";
import type { PageId, IELTSReadingPart } from "./types";

interface LinkedReadingPartsProps {
  readingId: string;
  onNavigate: (page: PageId, data?: Record<string, string>) => void;
}

export default function LinkedReadingParts({
  readingId,
  onNavigate,
}: LinkedReadingPartsProps) {
  const [linkedParts, setLinkedParts] = useState<IELTSReadingPart[]>([]);
  ...
  const loadLinkedParts = useCallback(async () => {
    setLoading(true);
    try {
      const [linkedRes, readingRes] = await Promise.all([
        ieltsReadingAPI.getLinkedParts(readingId),
  ...
  const handleUnlink = async (partId: string) => {
    if (!confirm("Unlink this reading part from the reading section?")) return;
    setUnlinking(partId);
    try {
      await ieltsReadingAPI.unlinkPart({
  ...
  const handleLink = async (partId: string, order?: number) => {
    await ieltsReadingAPI.linkPart({
      reading_id: readingId,
      reading_part_id: partId,
      order,
    });
  ...
  return (
    <Box>
      ...
      <HStack gap={1.5} fontSize="sm" color="gray.400" mb={4}>
        ...
        <Text>{readingTitle || "Reading"}</Text>
        <Text color="gray.300" _dark={{ color: "gray.600" }}>
          /
        </Text>
      ...
      <Flex justifyContent="space-between" alignItems="center" mb={4}>
        ...
        <HStack gap={2}>
          ...
          <Button
            ...
            color="white"
            _hover={{ bg: "#3730a3" }}
            onClick={() => onNavigate("reading-part-form", { readingId })}
          >
      ...
      {linkedParts.length === 0 ? (
        <Box textAlign="center" py={12} color="gray.400">
          ...
          <Text fontSize="sm">
            Link existing reading parts or create new ones to add to this
            reading section.
          </Text>
        </Box>
      ) : (
                    ...
                    >
                      <HStack gap={1}>
                        ...
                        <Text fontSize="sm" fontWeight="600">
                          {(p as IELTSReadingPart & { _junctionOrder?: number })
                    ...
                    >
                      <HStack gap={1}>
                        ...
                        <IconButton
                          ...
                          aria-label="Unlink"
                          title="Unlink from this reading"
                        >
                          <Unlink size={14} />
                        </IconButton>
                      </HStack>
